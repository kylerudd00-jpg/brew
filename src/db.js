 ...
 *
 * All queries are synchronous (better-sqlite3 design) — safe to call inside
 * async Express handlers since Node's event loop isn't blocked for meaningful time.
 */
...
function getZipCoords(zip) {
  return _getZip.get(zip) || null;
}

function saveZipCoords(zip, lat, lng) {
  _upsertZip.run({ zip, lat, lng });
}
...
// Returns breweries within a bounding box (quick filter; haversine done in JS)
...
function getBrewery(id) {
  return _getBrewery.get(id) || null;
}

/** Returns breweries from DB near coords, fresher than maxAgeSec seconds. */
...
const upsertBreweries = db.transaction((list) => {
  for (const b of list) upsertBrewery(b);
});
...
const _getBeersByBrewery = db.prepare(
  'SELECT * FROM beers WHERE brewery_id = ? ORDER BY rating DESC'
);
...
function upsertBeer(beer) {
  ...
  return id;  // stable deterministic ID — safe to return and use in responses
}

const upsertBeers = db.transaction((list) => {
  for (const b of list) upsertBeer(b);
});

function getBeersByBrewery(breweryId) {
  return _getBeersByBrewery.all(breweryId);
}
...
const _getEventsByBrewery = db.prepare(
  `SELECT * FROM events WHERE brewery_id = ? AND (date IS NULL OR date >= date('now'))
   ORDER BY date ASC LIMIT 10`
);

const _upcomingEvents = db.prepare(
  `SELECT * FROM events WHERE (date IS NULL OR date >= date('now'))
   AND last_updated > unixepoch() - 86400
...
const upsertEvents = db.transaction((list) => {
  for (const ev of list) upsertEvent(ev);
});

function getEventsByBrewery(breweryId) {
  return _getEventsByBrewery.all(breweryId);
}

function getUpcomingEvents(limit = 20) {
  return _upcomingEvents.all({ limit });
}

// ── Individual beer lookup ────────────────────────────────────────────────────
...
const _similarBeers = db.prepare(`
  ...
  FROM beers b
  JOIN breweries br ON b.brewery_id = br.id
  WHERE b.style_category = @style
    AND b.id != @id
    AND b.available = 1
    AND b.last_seen > unixepoch() - 86400 * 14
  ORDER BY b.rating DESC
  LIMIT @limit
`);

function getBeer(id) {
  return _getBeerById.get(id) || null;
}

function getSimilarBeers(id, style, limit = 5) {
  return _similarBeers.all({ id, style, limit });
}
...
const _deleteFavorite = db.prepare(
  'DELETE FROM favorites WHERE session_id = ? AND beer_id = ?'
);

const _getFavorites = db.prepare(
  'SELECT * FROM favorites WHERE session_id = ? ORDER BY saved_at DESC'
);

const _isFavorite = db.prepare(
  'SELECT 1 FROM favorites WHERE session_id = ? AND beer_id = ?'
);

const _clearFavorites = db.prepare(
...
function deleteFavorite(sessionId, beerId) {
  return _deleteFavorite.run(sessionId, beerId).changes > 0;
}

function getFavorites(sessionId) {
  return _getFavorites.all(sessionId);
}

function isFavorite(sessionId, beerId) {
  return !!_isFavorite.get(sessionId, beerId);
}

function clearFavorites(sessionId) {
  return _clearFavorites.run(sessionId).changes;
}
...
const _searchStats = db.prepare(`
  ...
  FROM searches
  WHERE searched_at > unixepoch() - 86400 * 7
`);
...
function logSearch(zip, { breweryCount, beerCount, durationMs, cacheHit }) {
  try {
    _logSearch.run({
      zip,
      brewery_count: breweryCount || 0,
      beer_count:    beerCount    || 0,
...
function getStats() {
  ...
  `).get();
  const weekly = _searchStats.get();
  const topZips = _popularZips.all();
  return { counts, weekly, topZips };
}
...
function cleanBeerName(name) {
  return name
    .replace(/\s*\d+(\.\d+)?%\s*(abv)?/gi, '')  // strip ABV
    .replace(/\s+/g, ' ')
    .trim();
}
